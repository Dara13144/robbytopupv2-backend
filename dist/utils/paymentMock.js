...
exports.generateABASignature = generateABASignature;
exports.generateABAMockPayment = generateABAMockPayment;
exports.calculateCRC16 = calculateCRC16;
exports.generateBakongKHQR = generateBakongKHQR;
 ...
 */
function generateABASignature(reqTime, merchantId, tranId, amount, items, shipping, type, paymentOption, apiKey) {
    ...
    return crypto_1.default.createHmac('sha256', apiKey).update(data).digest('base64');
}
/**
 * Generates mock ABA checkout payload and link
 */
function generateABAMockPayment(tranId, amount, itemName, merchantId, apiKey, baseUrl) {
    ...
    const hash = generateABASignature(reqTime, merchantId, tranId, amountStr, items, '0', // shipping
    'purchase', // type
...
async function generateBakongKHQR(tranId, amount, itemName) {
    ...
    const md5 = crypto_1.default.createHash('md5').update(rawKHQR).digest('hex').toLowerCase().trim();
 ...
 */
async function checkBakongPaymentStatus(md5, khpayTxnId, ctx) {
 ...
 */
function verifyBakongWebhook(md5Hash, transactionId, signature, apiKey) {
    const dataToSign = `${md5Hash}${transactionId}`;
    const expectedSig = crypto_1.default.createHmac('sha512', apiKey).update(dataToSign).digest('hex');
    if (!signature || expectedSig.length !== signature.length) {
        return false;
    }
    return crypto_1.default.timingSafeEqual(Buffer.from(expectedSig, 'hex'), Buffer.from(signature, 'hex'));
}
